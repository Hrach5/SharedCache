...
export class RedisService {
  ...
  async deleteLessUsedData(keys: string[]) {
    const firstData = JSON.parse(
      await this.redisClient.getClient().get(keys[0]),
    );
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (data.count <= firstData.count) {
        ...
        firstData.id = data.id;
      }
    }

    await this.redisClient.getClient().del();
  }
  ...
  async put(key: string, value: any): Promise<boolean> {
    ...
    if (keys.length > 15) this.deleteLessUsedData(keys);

    await this.redisClient.getClient().set(key, JSON.stringify(data));

    await this.redisClient
      .getClient()
      .expire(key, +process.env.REDIS_CACHE_TTL || 3600);
